...
// @ts-ignore
import { vitePluginGenerateWebextLocales } from "./app/vite/plugin/generateWebextLocales.ts";

import git from "git-rev-sync";
import pkg from "./package.json";

function base_path(_path = "") {
  return path.resolve(__dirname, "app", _path);
}
    ...
    webExtension({
      ...
      manifest: () => ({
        ...
        web_accessible_resources: [
          {
            resources: ["icons/*", "lib/*", "pt-plugin-plus-mv3.css"],
            matches: ["*://*/*"],
          },
        ],
      }),
      ...
      watchFilePaths: [path.resolve(__dirname, "package.json")],
      htmlViteConfig: {
        plugins: [
          {
            name: "sort-asserts",
