...
**Architecture:** 先以安全反向变更撤回 `4512a718`，随后只重建共享表格、顶部滚动条和各页面角色映射。表格工具层负责不可变表头归一化，组件层负责滚动同步，主题 CSS 负责 action 隔离；设置导航通过默认子路由解析和合法 Tab 归一化消除父路由空状态。
...
### Task 4: 将全部现有表格映射为主列、次列和 action 列
