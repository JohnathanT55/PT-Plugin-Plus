...
assert(
  state.hostToSiteId["old-tracker.example.invalid"] === siteId,
  "former host maps to the same siteId"
);
assert(
...
assert(
  validateMV3State({ metadata: { schemaVersion: 1 } } as any).some(
...
assert(
  Object.keys(state.userHistory[siteId]).some(key =>
    key.indexOf("__conflict__:") === 0
  ),
...
assert(
  state.downloaders["qb-fixture"].address ===
    "https://downloader.example.invalid",
  "download server address is retained"
);
assert(
  !Object.prototype.hasOwnProperty.call(
    state.sites[siteId].legacyConfig,
    "script"
  ),
  "legacy executable site fields remain only in untouched MV2 storage"
);
assert(
  state.backupServers["webdav-fixture"].type === "WebDAV",
  "WebDAV configuration is retained"
);
assert(
...
assert(
  state.searchSnapshots[0].result![0].siteId === siteId,
  "search snapshot results receive siteId annotations"
);
assert(
...
assert(
  resolution.target.defaultDirectory === "/fixture/site-a" &&
    !resolution.requiresSelection,
  "legacy first folder remains the direct-push default while all folders are retained"
);

delete state.siteDownloadProfiles[siteId].byDownloader["qb-fixture"]
  .defaultDirectory;
...
state.siteDownloadProfiles[siteId].byDownloader["qb-fixture"].directories = [
  "/fixture/site-a"
];
...
const incompleteResolution = resolveDownloadTarget(
  undefined,
  undefined,
  {},
  state.downloaders,
  noDownloaderSettings
);
assert(
...
assert(
  knownSiteAlias.state.sites.hdkylin.activeHost === "hdkyl.in" &&
    knownSiteAlias.state.sites.hdkylin.legacyConfig.passkey ===
      "fixture-passkey",
  "an official host alias never overwrites the canonical site record"
);
assert(
  knownSiteAlias.state.sites.hdkylin.legacyAliasConfigs![0].passkey ===
    "fixture-alias-passkey" &&
    knownSiteAlias.state.siteDownloadProfiles.hdkylin.defaultDownloaderId ===
      "qb-alias",
...
assert(
  validateMV3State(knownSiteAlias.state).filter(item => item.severity === "error")
    .length === 0,
...
[
  ...
  assert(
    migrated.state.hostToSiteId[host] === "mteam",
    `M-Team alias ${host} uses the PT-depiler siteId`
  );
});
...
async function verifyRepositoryMigration() {
  ...
  const repository = new MV3Repository(
    new ChromeStorageAdapter(memory),
    () => 1000
  );
  const migratedState = await repository.initialize();
  assert(
    migratedState.metadata.schemaVersion === 1,
    "repository initializes the current schema"
  );
  assert(
    !!memory.values[MV3_STORAGE_KEYS.metadata],
    "repository commits the migration marker"
  );
  const revision = memory.values[MV3_STORAGE_KEYS.metadata].storageRevision;
  assert(
    MV3_DATA_STORAGE_KEYS.every(
      key => !!memory.values[revisionedStorageKey(key, revision)]
    ),
    "repository commits a complete immutable storage revision"
  );
  assert(
    !!memory.values[LEGACY_STORAGE_KEYS.config],
    "repository leaves legacy keys untouched"
  );

  const reloaded = await new MV3Repository(
    new ChromeStorageAdapter(memory),
    () => 1001
  ).initialize();
  assert(
    reloaded.metadata.storageRevision === revision,
    "repository reloads the exact committed revision"
  );

  const incomplete = new MemoryStorageArea(memory.values);
  delete incomplete.values[
    revisionedStorageKey(MV3_STORAGE_KEYS.sites, revision)
  ];
  let incompleteRejected = false;
  try {
    await new MV3Repository(
      new ChromeStorageAdapter(incomplete),
      () => 1002
    ).initialize();
  } catch (error) {
  ...
  assert(
    incompleteRejected,
    "missing committed partitions fail closed instead of becoming empty data"
  );
  ...
  try {
    await new MV3Repository(
      new ChromeStorageAdapter(olderSchema),
      () => 1003
    ).initialize();
  } catch (error) {
    olderSchemaRejected =
      String(error).indexOf("No MV3 state migration registered") !== -1;
  }
  assert(
    olderSchemaRejected &&
      olderSchema.values[MV3_STORAGE_KEYS.metadata].schemaVersion === 0,
  ...
  const revisionTwoRepository = new MV3Repository(
    new ChromeStorageAdapter(memory),
    () => 2000
  );
  ...
  const revisionTwo =
    memory.values[MV3_STORAGE_KEYS.metadata].storageRevision;
  await revisionTwoRepository.writeState(revisionTwoState);
  const revisionThree =
    memory.values[MV3_STORAGE_KEYS.metadata].storageRevision;
  assert(
    !memory.values[revisionedStorageKey(MV3_STORAGE_KEYS.sites, revision)] &&
      !!memory.values[
        revisionedStorageKey(MV3_STORAGE_KEYS.sites, revisionTwo)
      ] &&
      !!memory.values[
        revisionedStorageKey(MV3_STORAGE_KEYS.sites, revisionThree)
      ],
