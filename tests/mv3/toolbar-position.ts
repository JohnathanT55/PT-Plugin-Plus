...
const toolbar = { width: TOOLBAR_DEFAULT_WIDTH, height: 360 };
const wideViewport = { width: 1920, height: 1080 };

const defaultCoordinates = placementToCoordinates(undefined, wideViewport, toolbar);
assert(defaultCoordinates.x === 1808, "the 96 px toolbar defaults to 16 px from the right edge");
assert(defaultCoordinates.y === 360, "the default position is vertically centered");
...
assert(leftCoordinates.y === 180, "vertical placement is relative to available viewport height");

const draggedLeft = coordinatesToPlacement({ x: 30, y: 180 }, wideViewport, toolbar);
assert(
  draggedLeft.dockSide === "left" && draggedLeft.edgeOffset === 30,
  "dragging near the left edge changes the global dock side",
);

const draggedRight = coordinatesToPlacement({ x: 1784, y: 540 }, wideViewport, toolbar);
assert(
...
assert(resizedCoordinates.x === 1144, "a narrower viewport preserves the selected right edge");
assert(resizedCoordinates.y === 330, "a shorter viewport preserves the vertical ratio");
...
assert(migrated.verticalRatio === 200 / 720, "a valid legacy Y position is preserved proportionally");
